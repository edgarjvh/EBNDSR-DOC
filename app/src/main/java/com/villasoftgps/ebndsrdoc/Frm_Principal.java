...
public class Frm_Principal extends Activity {
    ...
    private static Spinner cboRepresentantes;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (sPrefs.getString(PROPERTY_USER,"").equals("")){
            ...
            docente = gson.fromJson(user, Docente.class);
        }
        ...
        cboRepresentantes = (Spinner) findViewById(R.id.cboRepresentantes);

        cboRepresentantes.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                cboRepresentantes.post(new Runnable() {
                    @Override
                    public void run() {
                        View view = cboRepresentantes.getSelectedView();

                        if (view != null){
                            TextView lblIdRepresentante = (TextView) view.findViewById(R.id.lblIdRepresentante);
                            int idRepresentante = Integer.parseInt(lblIdRepresentante.getText().toString());

                            SharedPreferences.Editor sEditor = sPrefs.edit();
                            sEditor.putInt(PROPERTY_CURRENT_ID_REP,idRepresentante);
                            sEditor.apply();
                            ...
                            if (!sPrefs.getString(PROPERTY_CONVERSATIONS,"").equals("")){
                                ...
                                for (int x = 0; x < items.size(); x++){
                                    if (items.get(x).getIdRepresentante() == idRepresentante && items.get(x).getIdDocente() == docente.getId()){
        ...
        btnEnviarMensaje.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!txtMensaje.getText().toString().equals("")){
                    View viewRepresentante = cboRepresentantes.getSelectedView();

                    if (viewRepresentante != null){
                        TextView lblRegistrado = (TextView) viewRepresentante.findViewById(R.id.lblRegistrado);
                        TextView lblIdRepresentante = (TextView) viewRepresentante.findViewById(R.id.lblIdRepresentante);
                        int idRepresentante = Integer.parseInt(lblIdRepresentante.getText().toString());
                        int registrado = Integer.parseInt(lblRegistrado.getText().toString());

                        if (registrado == 0){
                            mostrarMensaje(false,1,"Este representante no ha sido registrado en el sistema\nMensaje no enviado!");
                            return;
                        }
        ...
        rgroupOpcionesCalendario.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                ...
                new AsyncCalendario().execute(docente.getId(),value);
            }
        });

        mensaje = "Buscando eventos para esta semana. Por favor espere...";
        new AsyncCalendario().execute(docente.getId(), 0);
        new AsyncCargarAlumnosRepresentantes().execute(docente.getId());
        new RegistrarGCM(Frm_Principal.this,docente);
    }
        ...
        try{
            if (MensajesItems.size() > 0){
                for (int i = 0; i < MensajesItems.size(); i++){
                    ...
                    View representante = cboRepresentantes.getSelectedView();

                    if (docente != null){
                        TextView lblIdRepresentante = (TextView)representante.findViewById(R.id.lblIdRepresentante);
                        idRepresentante = Integer.parseInt(lblIdRepresentante.getText().toString());
                    }

                    if ((MensajesItems.get(i).getVia() == 1  && MensajesItems.get(i).getIdRepresentante() == idRepresentante)
            ...
            public void run() {
                ...
                if (view != null){
                    ...
                    if (!sPrefs.getString(PROPERTY_CONVERSATIONS,"").equals("")){
                        ...
                        for (int x = 0; x < items.size(); x++){
                            if (items.get(x).getIdDocente() == docente.getId() && items.get(x).getIdRepresentante() == idRepresentante){
                                MensajesItems.add(items.get(x));
                            }
                        }
    ...
    public static void actualizarRegistroRepresentante(final int idRepresentante, final int registrado){
        principal.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (spinnerItems.size() > 0){
                    for (int i = 0; i < spinnerItems.size(); i++){
                        if (spinnerItems.get(i).getIdRepresentante() == idRepresentante){
    ...
    public static void cerrarSesion(){
        ...
        Gson gson = new Gson();
        Log.d("EJVH", "Cerrando la Sesion Docente");
        Docente docente = gson.fromJson(sPrefs.getString(PROPERTY_USER,""),Docente.class);

        new AsyncEliminarGcm().execute(docente.getId(),0);
    ...
    private class AsyncCargarAlumnosRepresentantes extends AsyncTask<Object,Integer, Integer>{
                ...
                switch (result) {
                    case "OK":
                        ...
                        for (int i = 0; i < array.length(); i++) {
                            ...
                            JSONObject representante = new JSONObject(alumno.getString("Representante"));

                            spinnerItems.add(new SpinnerItems(
                                    representante.getInt("Registrado"),
                                    representante.getInt("Id"),
                                    representante.getString("Apellidos"),
                                    representante.getString("Nombres"),
                                    alumno.getInt("Id"),
                                    alumno.getString("Apellidos"),
                                    alumno.getString("Nombres"),
                                    representante.getString("Imagen")
                            ));
                        }
                        publishProgress(1);
                        break;
        ...
        protected void onProgressUpdate(Integer... values) {
            ...
            if (values[0] == 1){
                ...
                cboRepresentantes.setAdapter(spinnerAdapter);
                ...
                if (spinnerItems.size() > 0){
                    ...
                    if(extras != null){
                        int idRepresentante = extras.getInt("incoming");
                ...
                cboRepresentantes.setSelection(sel);
